...
export default function page() {
  let [stickyContent, setStickyContent] = useState<StickyInterface>({ title: '', text: '' });
...
function BabelStickies({ stickies: setStickies }: {stickies : StickyInterface[]}) {
  ...
  useEffect(() => {
    for (let i = 0; i < setStickies.length; i++) {
      stickiesArr.push(<Sticky text={setStickies[i].text} title={setStickies[i].text} closeFunction={() => {
      setStickiesArr(stickiesArr.slice(0, i).concat(stickiesArr.slice(i+1, stickiesArr.length)))
    ...
    return <div className='flex flex-wrap justify-stretch border gap-6'>{ stickiesArr.map(element => element)}</div>;
  }
