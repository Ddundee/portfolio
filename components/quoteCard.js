...
export default function QuoteCard({}) {
  ...
  return (
    <div className='border-[#EB9393] bg-[#fbe8e8] border p-6 rounded-[6px] w-max text-[#170202] font-geist_sans'>
      <h1 className='font-semibold text-lg text-bl '>
        Quote from my <span className='underline'>GOAT</span>!
      </h1>
      <div className='w-full h-2' />
      {curQuote !== undefined ? (
        <p className=' font-normal text-sm w-[464px] h-max'>
          {quotes[curQuote]}
        </p>
      ) : (
        <Skeleton height={12} />
      ...
      <div className='flex justify-end w-full h-full'>
        <Button
          action={() => {
            setCurQuote((curQuote + 1) % quotes.length);
          }}
        >
          Next Quote &rarr;
        </Button>
